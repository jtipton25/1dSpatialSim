...
%
For $j = 1, \ldots, p$ and using the fact that $\bm{X}_i^T \bm{X}_j = 0$ (by orthogonality of principal components), %$\beta_j = \left( \bm{X}_{j}^T \bm{X}_{j} \right)^{ - 1} \bm{X}_{j}^T \bm{y}_c$ and $\bm{X}_{j}^T \bm{X}_{j} = \delta_j$ 
%
\begin{align*}
%
