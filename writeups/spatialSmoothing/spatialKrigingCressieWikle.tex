...
\begin{align*}
  \left( \bm{z}^{ \left( k \right) }_t \left( \bm{s}_0 \right) \middle| \bm{y} \right) &  = \bm{X} \left( \bm{s}_0 \right) ^T \bm{\beta}^{ \left( k \right ) }_t + \bm{c}_Y \left( \bm{s}_0 \right )^T \bm{\Sigma}^{ - 1}
\end{align*}
% 
