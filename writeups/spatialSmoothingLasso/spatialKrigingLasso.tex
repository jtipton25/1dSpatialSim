...
%
& \propto (\prod_{t = 1}^T |\bm{\Sigma} |^{ - \frac{1} {2}}) e^{ - \frac{1} {2} \sum_{t = 1}^T (\bm{y}_t - \bm{H}_t \bm{X} \bm{\beta}_t)^T \bm{\Sigma}^{ - 1} (\bm{y}_t - \bm{H}_t \bm{X} \bm{\beta}_t)} |\bm{\Sigma}_\beta |^{ - \frac{1} {2}} e^{ - \frac{1} {2} \sum_{t = 1}^T \bm{\beta}_t^T \bm{\Sigma}_{\beta}^{ - 1} \bm{\beta}_t} (\sigma^2_\epsilon)^{ - \alpha_\epsilon + 1} e^{ - \frac{\beta_\epsilon} {\sigma^2_\epsilon}}\\
%
& \propto |\bm{\Sigma} |^{ - \frac{T} {2}} e^{ - \frac{1} {2} \sum_{t = 1}^T (\bm{y}_t - \bm{H}_t \bm{X} \bm{\beta}_t)^T \bm{\Sigma}^{ - 1} (\bm{y}_t - \bm{H}_t \bm{X} \bm{\beta}_t)} |\bm{\Sigma}_\beta |^{ - \frac{1} {2}} e^{ - \frac{1} {2} \sum_{t = 1}^T \bm{\beta}_t^T \bm{\Sigma}_{\beta}^{ - 1} \bm{\beta}_t} (\sigma^2_\epsilon)^{ - \alpha_\epsilon + 1} e^{ - \frac{\beta_\epsilon} {\sigma^2_\epsilon}}\\
\end{align*}
% 
which can be sampled using a Metropolis-Hastings step
%
