...
#   make.R.list <- function(s, sigma.squared.eta, phi, D.list){#, L.list){
...
#   
  ...
  make.L.list <- function(s, I.nt, HX.list, tHX.list){
    e <- eigen(I.nt[[s]] - HX.list[[s]] %*% solve(tHX.list[[s]] %*% HX.list[[s]]) %*% tHX.list[[s]])
...
#   tLL.list <- lapply(1:t, make.tLL.list, L.list = L.list, tL.list = tL.list)  
  ...
  ##
    ...
    if(sigma.squared.eta.star > 0){
      R.list.star <- lapply(1:t, make.R.list, sigma.squared.eta = sigma.squared.eta.star, phi = phi, D.list = D.list, LtL.list = LtL.list)
