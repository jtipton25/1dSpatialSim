...
## params = c(mu.0, Sigma.0, alpha.beta, beta.beta, alpha.eta, beta.eta, alpha.epsilon, beta.epsilon, alpha.phi, beta.phi, n.mcmc = 5000)
  ...
  make.identity.list.full <- function(s, nt){
    diag(ncells - nt[s])
  }
  ...
  I.full.t <- lapply(1:t, make.identity.list.full, nt = nt)
  I.full <- diag(ncells)
  ...
  if(k > n.burn){
    if(k %% 10 == 0){
      ...
      fort.raster.tmp <- sapply(1:t, make.fort.batch, beta = beta, c.Y = c.Y, Sigma.inv = Sigma.inv)
