...
mcmc.1d <- function(Y.list, H.list, X, locs, n.mcmc, mu.0, Sigma.0, sigma.squared.beta, sigma.squared.eta, alpha.epsilon, beta.epsilon, alpha.beta, beta.beta, alpha.phi, beta.phi, mu.beta,  s.star, sigma.squared.eta.tune, sigma.squared.epsilon.tune, phi.tune){
  ...
  D.list <- lapply(1, make.D.list, H.list = H.list, locs = locs)
  R.list <- lapply(1, make.R.list, sigma.squared.eta = sigma.squared.eta, phi = phi, D.list = D.list)
  
  sigma.squared.epsilon <- 1 / rgamma(1, alpha.epsilon, beta.epsilon)
  I.nt <- lapply(1, make.identity.list, nt = nt)
  Sigma.epsilon <- lapply(1, make.Sigma.epsilon, sigma.squared.epsilon = sigma.squared.epsilon, I.nt = I.nt)
  Sigma.epsilon.inv <- lapply(1, make.Sigma.epsilon.inv, sigma.squared.epsilon = sigma.squared.epsilon, I.nt = I.nt)
  
  Sigma.0 <- sigma.squared.0 * diag(tau)
  Sigma.0.inv <- solve(Sigma.0)
  
  Sigma <- lapply(1, make.Sigma, R.list = R.list, Sigma.epsilon = Sigma.epsilon)
  Sigma.inv <- lapply(1, make.Sigma.inv, Sigma = Sigma)
  ...
  for(k in 1:n.mcmc){
    ...
    if(sigma.squared.eta.star > 0){
      R.list.star <- lapply(1, make.R.list, sigma.squared.eta = sigma.squared.eta.star, phi = phi, D.list = D.list)
      Sigma.star <- lapply(1, make.Sigma, R.list = R.list.star, Sigma.epsilon = Sigma.epsilon)
      Sigma.star.inv <- lapply(1, make.Sigma.inv, Sigma = Sigma.star)
      mh.eta.1 <- sum(sapply(1, make.mh, beta = beta, Sigma = Sigma.star, Sigma.inv = Sigma.star.inv)) + dinvgamma(sigma.squared.eta.star, alpha.eta, beta.eta, log = TRUE)
      mh.eta.2 <- sum(sapply(1, make.mh, beta = beta, Sigma = Sigma, Sigma.inv = Sigma.inv)) + dinvgamma(sigma.squared.eta, alpha.eta, beta.eta, log = TRUE)
    ...
    if(sigma.squared.epsilon.star > 0){
      Sigma.epsilon.star <- lapply(1, make.Sigma.epsilon, sigma.squared.epsilon = sigma.squared.epsilon.star, I.nt = I.nt)
      Sigma.epsilon.star.inv <- lapply(1, make.Sigma.epsilon.inv, sigma.squared.epsilon = sigma.squared.epsilon, I.nt = I.nt)
      Sigma.star <- lapply(1, make.Sigma, R.list = R.list, Sigma.epsilon = Sigma.epsilon.star)
      Sigma.star.inv <- lapply(1, make.Sigma.inv, Sigma = Sigma.star)
      mh.epsilon.1 <-	sum(sapply(1, make.mh, beta = beta, Sigma = Sigma.star, Sigma.inv = Sigma.star.inv)) + dinvgamma(sigma.squared.epsilon.star, alpha.epsilon, beta.epsilon, log = TRUE)
      mh.epsilon.2 <- sum(sapply(1, make.mh, beta = beta, Sigma = Sigma, Sigma.inv = Sigma.inv)) + dinvgamma(sigma.squared.epsilon, alpha.epsilon, beta.epsilon, log = TRUE)
    ...
    if(phi.star > 0){
      R.list.star <- lapply(1, make.R.list, sigma.squared.eta = sigma.squared.eta, phi = phi.star, D.list = D.list)	  
      Sigma.star <- lapply(1, make.Sigma, R.list = R.list.star, Sigma.epsilon = Sigma.epsilon)
      Sigma.star.inv <- lapply(1, make.Sigma.inv, Sigma = Sigma.star)
      mh.phi.1 <-	sum(sapply(1, make.mh, beta = beta, Sigma = Sigma.star, Sigma.inv = Sigma.star.inv)) + dinvgamma(phi.star, alpha.phi, beta.phi, log = TRUE)
      mh.phi.2 <- sum(sapply(1, make.mh, beta = beta, Sigma = Sigma, Sigma.inv = Sigma.inv)) + dinvgamma(phi, alpha.phi, beta.phi, log = TRUE)
    ...
    if(k > n.burn){
      ...
      fort.raster <- fort.raster + (1 / (n.mcmc - n.burn)) * sapply(1, make.fort.batch, beta = beta, chol.Sigma = chol.Sigma, ncells = ncells)
