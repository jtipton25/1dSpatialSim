...
mcmc.1d <- function(Y.list, H.list, X, locs, n.mcmc, mu.0, Sigma.0, alpha.epsilon, beta.epsilon, alpha.beta, beta.beta, alpha.phi, beta.phi, mu.beta, sigma.squared.eta.tune, sigma.squared.epsilon.tune, phi.tune, s.star){
  ...
  make.Sigma.inv <- function(s, Sigma.epsilon.inv, C.star, c){
    cH.list <- c[, H.list[[s]]]
...
#   }
  ...
  Sigma <- lapply(1:t, make.Sigma, Sigma.epsilon = Sigma.epsilon, C.star.inv = C.star.inv, c = c)
  Sigma.inv <- lapply(1:t, make.Sigma.inv, Sigma.epsilon.inv = Sigma.epsilon.inv, C.star = C.star, c = c)
  ...
  for(k in 1:n.mcmc){
    ...
    if(sigma.squared.eta.star > 0){
      ...
      Sigma.star <- lapply(1:t, make.Sigma, Sigma.epsilon = Sigma.epsilon, C.star.inv = C.star.star.inv, c = c.star)
      Sigma.star.inv <- lapply(1:t, make.Sigma.inv, Sigma.epsilon = Sigma.epsilon, C.star = C.star.star, c = c.star)
    ...
    if(sigma.squared.epsilon.star > 0){
      ...
      Sigma.star <- lapply(1:t, make.Sigma, Sigma.epsilon = Sigma.epsilon.star, C.star.inv = C.star.inv, c = c)
      Sigma.star.inv <- lapply(1:t, make.Sigma.inv, Sigma.epsilon = Sigma.epsilon.star, C.star = C.star, c = c)
    ...
    if(k > n.burn){
      if(k %% 10 == 0){
        fort.raster.tmp <- sapply(1:t, make.fort.batch, beta = beta, c = c, C.star = C.star, C.star.inv = C.star.inv, sigma.squared.epsilon = sigma.squared.epsilon)#, w.tilde = w.tilde)
