...
#    Sigma.epsilon.inv[[s]] - Sigma.epsilon.inv[[s]] %*% t(cH.list) %*% solve(C.star.inv + cH.list %*% Sigma.epsilon.inv[[s]] %*% t(cH.list)) %*% cH.list %*% Sigma.epsilon.inv[[s]]
  ...
  make.fort.batch <- function(s, beta, c, C.star, C.star.inv, sigma.squared.epsilon){
    w.star <- rmvnorm(1, vec.0, C.star) # this should be c.star using the notation but c.star is taken by the mh step
    w.tilde <- t(c) %*% C.star.inv %*% t(w.star) #likewise, this is c.tilde
  	if(dim(beta)[1] == 1){
  ...
  Sigma.inv <- lapply(1:t, make.Sigma.inv, Sigma.epsilon.inv = Sigma.epsilon, C.star = C.star.inv, c = c, H.list = H.list)
  ...
  ##
    ...
    if(k > n.burn){
      if(k %% 10 == 0){
        fort.raster <- fort.raster + 1 / ((n.mcmc - n.burn) / 10) * sapply(1:t, make.fort.batch, beta = beta, c = c, C.star = C.star, C.star.inv = C.star.inv, sigma.squared.epsilon = sigma.squared.epsilon)
        if(k %% 1000 == 0){
